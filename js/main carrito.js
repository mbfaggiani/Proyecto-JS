const renderProductosCarrito = () => {
    ...
    if (totalCarrito() > 0) {    
        ...
        <tbody>
        <tr>
        <td colspan="6" class="text-end"><a href="#" class="btn btn-info" onClick="vaciarCarrito()">Vaciar Carrito <img src="images/cesto.png" alt="Vaciar Carrito" width="16" /></a></td>
        </tr>`;
        ...
        for (let producto of productos_carrito) {
            ...
            <td class="align-middle"><button class="btn btn-info rounded-circle" title="Eliminar Item" onClick="eliminarItemProducto(${producto.id});"> - </button> ${producto.cantidad} <button class="btn btn-info rounded-circle" title="Agregar Item" onClick="agregarItemProducto(${producto.id});"> + </button></td>
